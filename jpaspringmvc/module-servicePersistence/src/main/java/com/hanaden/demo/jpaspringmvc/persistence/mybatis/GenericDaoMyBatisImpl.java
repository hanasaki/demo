 ...
 */
public abstract class GenericDaoMyBatisImpl<T_OBJ, T_PK>
        implements GenerickDaoMyBatisInterface<T_OBJ, T_PK> {

    private static Logger log = Logger.getLogger(GenericDaoMyBatisImpl.class);
    private static final String NAMESPACE = "mappers";
    private SqlSessionFactory sf; //reference to mybatis session factory 
    private Class<T_OBJ> type;
    /**
     ...
     * Default Constructor
     */
    public GenericDaoMyBatisImpl(Class<T_OBJ> type, SqlSessionFactory sf) {
        this.type = type;
        this.sf = sf;
        if (sf == null) {
    ...
    public T_OBJ get(T_PK id) throws PersistenceException {
        ...
        try {
            ...
            obj = (T_OBJ) session.selectOne(query, id);
        } finally {
            session.close();
        }
        return obj;
    ...
    public ArrayList<T_OBJ> getAll() throws PersistenceException {
        ...
        SqlSession session = sf.openSession();
        ArrayList<T_OBJ> list = null;
        try {
            String query = NAMESPACE + "." + PREFIX_SELECT_QUERY + "All" + this.type.getSimpleName();
            list = (ArrayList<T_OBJ>) session.selectList(query);
        } finally {
            session.close();
        }
        return list;
    ...
    public T_OBJ getByName(String name) throws PersistenceException {
        ...
        try {
            ...
            obj = (T_OBJ) session.selectOne(query, name);
        } finally {
            session.close();
        }
        return obj;
